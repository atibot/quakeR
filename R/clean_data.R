...
#' month, day and converting it to the Date class LATITUDE and LONGITUDE columns
...
eq_clean_data <-
    function(raw){
        df <-
            raw %>%
            dplyr::mutate_at(dplyr::vars("LATITUDE", "LONGITUDE",
                           "EQ_PRIMARY", "TOTAL_DEATHS"), as.numeric) %>%
            dplyr::mutate_at(dplyr::vars("MONTH","DAY"),
            ...
            dplyr::select(.data$DATE, .data$LATITUDE, .data$LONGITUDE,
